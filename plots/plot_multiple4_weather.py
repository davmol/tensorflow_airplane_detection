...
for e, row in enumerate(result):
    if row[0] == "mist":
        weather_status.append(12 / 2)
    if row[0] == "shower rain":
        weather_status.append(11 / 2)
    if row[0] == "light intensity shower rain":
        weather_status.append(10 / 2)
    if row[0] == "moderate rain":
        weather_status.append(9 / 2)
    if row[0] == "light rain":
        weather_status.append(8 / 2)
    if row[0] == "light intensity drizzle rain":
        weather_status.append(7 / 2)
    if row[0] == "light intensity drizzle":
        weather_status.append(6 / 2)
    if row[0] == "overcast clouds":
        weather_status.append(5 / 2)
    if row[0] == "broken clouds":
        weather_status.append(4 / 2)
    if row[0] == "scattered clouds":
        weather_status.append(3 / 2)
    if row[0] == "few clouds":
        weather_status.append(2 / 2)
    if row[0] == "clear sky":
        weather_status.append(1 / 2)

    weather_status_t.append(row[1])

print(weather_status_t)
print(weather_status)
print(min(weather_status), max(weather_status))
...
data = [[weather_status_t, weather_status, "Wetterstatus", cmap[16]],
        [object_t, object, "Objekterkennungen", cmap[0]]
        ]

object_lbl2 = ["" for i in range(0, 12)]
weather_lbl2 = object_lbl2
bar_width = 0.004
bar_width2 = 0.04
line_width = 0.3
...
def subplots(data):
    ...
    l = len(datetimes)
    for d in range(0, 2):
        fig, ax = plt.subplots(len(data), 4, figsize=(20, 10))
        ...
        for num, values in enumerate(data):
            ...
            if ylabel != "Fehlertypen" or ylabel == "Wetterstatus":
                ax[num, 0].set_ylim(min(y), max(y))
                ax[num, 0].yaxis.set_major_locator(MultipleLocator(0.5))
            else:
                ax[num, 0].set_ylim(min(y), max(y))
            ax[num, 0].set_xlim(datetimes[0 + d * 4] + datetime.timedelta(hours=6),
                                datetimes[0 + d * 4] + datetime.timedelta(hours=16))
            ...
            if ylabel != "Fehlertypen" or ylabel == "Wetterstatus":
                ...
                ax[num, 1].yaxis.set_major_locator(MultipleLocator(0.5))
            else:
                ax[num, 0].set_ylim(min(y), max(y))
            ax[num, 1].set_xlim(datetimes[1 + d * 4] + datetime.timedelta(hours=6),
                                datetimes[1 + d * 4] + datetime.timedelta(hours=16))
            ...
            if ylabel == "Wetterstatus":
                ax[num, 1].set_yticklabels(weather_lbl2)
            if ylabel == "Objekterkennungen":
                ax[num, 1].set_yticklabels(object_lbl2)
            ...
            if ylabel != "Fehlertypen" or ylabel == "Wetterstatus":
                ...
                ax[num, 2].yaxis.set_major_locator(MultipleLocator(0.5))
            else:
                ax[num, 0].set_ylim(min(y), max(y))
            ax[num, 2].set_xlim(datetimes[2 + d * 4] + datetime.timedelta(hours=6),
                                datetimes[2 + d * 4] + datetime.timedelta(hours=16))
            ...
            if ylabel == "Wetterstatus":
                ax[num, 2].set_yticklabels(weather_lbl2)
            if ylabel == "Objekterkennungen":
                ax[num, 2].set_yticklabels(object_lbl2)
            ...
            if ylabel != "Fehlertypen" or ylabel == "Wetterstatus":
                ...
                ax[num, 3].yaxis.set_major_locator(MultipleLocator(0.5))
            else:
                ax[num, 0].set_ylim(min(y), max(y))
            ax[num, 3].set_xlim(datetimes[3 + d * 4] + datetime.timedelta(hours=6),
                                datetimes[3 + d * 4] + datetime.timedelta(hours=16))
            ...
            if ylabel == "Wetterstatus":
                ax[num, 3].set_yticklabels(weather_lbl2)
            if ylabel == "Objekterkennungen":
                ax[num, 3].set_yticklabels(object_lbl2)

        fig.autofmt_xdate()
        plt.tight_layout()
        # plt.show()
        plt.savefig("multiplot_weather_ " + str(d) + " .png")
