...
def save_image_array_as_png(image, output_path):
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGBA')
...
def draw_bounding_box_on_image(image,
  ...
  try:
    font = ImageFont.truetype('/usr/share/fonts/truetype/quicksand/Quicksand-Medium.ttf', 40)
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for box, color in box_to_color_map.items():
    ...
    if ymin is not None and ymax is not None and xmin is not None and xmax is not None:
        return image, ymin, xmin, ymax,  xmax
    else:
        return image
